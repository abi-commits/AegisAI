...
class CaseManager:
    ...
    def __init__(
        self, cases_table: Optional[str] = None,
        ...
        if not self.cases_table_name:
            raise ValueError("Cases table required")
        if not self.evidence_bucket:
            raise ValueError("Evidence bucket required")
    ...
    def create_case(self, decision_id: str, session_id: str, user_id: str,
                    ai_action: str, ai_confidence: float, reason_for_review: str,
                    evidence: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            self.s3_client.put_object(
                Bucket=self.evidence_bucket, Key=evidence_key,
                Body=json.dumps(evidence, indent=2, default=str).encode(),
                ContentType="application/json",
                Metadata={"case-id": case_id, "decision-id": decision_id,
        ...
        case_item = {
            "pk": f"PK#CASE#{case_id}", "sk": f"SK#CASE#{now.isoformat()}",
            "case_id": case_id, "decision_id": decision_id, "session_id": session_id,
            "user_id": user_id, "status": CaseStatus.PENDING.value,
            "ai_action": ai_action, "ai_confidence": Decimal(str(ai_confidence)),
            "reason_for_review": reason_for_review,
            "evidence_uri": f"s3://{self.evidence_bucket}/{evidence_key}",
            "created_at": now.isoformat(), "history": [], "metadata": metadata or {},
            "gsi1_pk": "CASE#PENDING", "gsi1_sk": now.isoformat(),
            "gsi2_pk": f"USER#{user_id}", "gsi2_sk": now.isoformat()}
    ...
    def get_case(self, case_id: str) -> Optional[Dict[str, Any]]:
        """Get case details."""
        try:
            response = self.cases_table.get_item(Key={"pk": f"PK#CASE#{case_id}", "sk": f"SK#CASE#"})
            item = response.get("Item")
            if item and "ai_confidence" in item:
    ...
    def get_pending_cases(self, limit: int = DataConstants.DEFAULT_QUERY_LIMIT) -> List[Dict[str, Any]]:
    ...
    def get_user_cases(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
...
class ReviewUIBackend:
    ...
    def get_review_dashboard(self, reviewer_id: str) -> Dict[str, Any]:
        ...
        return {"pending_count": len(pending), "pending_cases": pending[:10], "stats": stats}
    
    def submit_review(self, case_id: str, action: str, reviewer_id: str,
                     reviewer_role: str, comment: Optional[str] = None,
                     new_action: Optional[str] = None) -> bool:
        """Submit review action."""
        try:
            review_action = ReviewAction[action.upper()]
            return self.case_manager.add_review_action(
                case_id=case_id, action=review_action, reviewer_id=reviewer_id,
                reviewer_role=reviewer_role, comment=comment, new_action=new_action)
        except KeyError:
